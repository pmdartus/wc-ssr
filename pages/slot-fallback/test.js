(() => {
    ...
    console.assert(slotComponent.shadowRoot.childElementCount === 1, 'Invalid number of child elements');
    
    const slot = slotComponent.shadowRoot.querySelector('slot');
    console.assert(slot.children.length === 1, '<slot> should have 1 child element');
    console.assert(slot.children[0].textContent === 'I am a fallback content', 'Invalid text content');
